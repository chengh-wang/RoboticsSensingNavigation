% EECE5554 Lab4 Data analysis
% by Chenghao Wang, 3-7-2022, Boston, MA
...
%list all rosbag
% circle_moving_bag = rosbag("./Data/moving.bag");
circle_moving_bag = rosbag("./Data/circle6.bag");

% circle_moving_bag = rosbag("./Data/static.bag");
%get topic
gps_topic = select(circle_moving_bag,"Topic","/gps_data");
imu_topic = select(circle_moving_bag,"Topic","/imu_data");
...
magneticfield(:,1) = cellfun(@(m) double(m.Magnetic.MagneticField_.X),imu_message);
magneticfield(:,2) = cellfun(@(m) double(m.Magnetic.MagneticField_.Y),imu_message);
magneticfield(:,3) = cellfun(@(m) double(m.Magnetic.MagneticField_.Z),imu_message);
...
% Yaw Pitch Roll
yaw = cellfun(@(m) double(m.Ypr.X),imu_message);
% yaw = yaw - yaw(1)
pitch = cellfun(@(m) double(m.Ypr.Y),imu_message);
roll = cellfun(@(m) double(m.Ypr.Z),imu_message);
...
hold on
plot3(xCorrected(:,1),xCorrected(:,2),xCorrected(:,3),'LineWidth',4)
grid on
hold off
...
yawFuse = yawFuse - yawFuse(1) + yaw(1) - 25;
...
title('Estimation or forward velocity');
...
%list all rosbag
moving_bag = rosbag("./Data/moving.bag");
% circle_moving_bag = rosbag("./Data/circle6.bag");

% circle_moving_bag = rosbag("./Data/static.bag");
%get topic
gps_topic = select(moving_bag,"Topic","/gps_data");
imu_topic = select(moving_bag,"Topic","/imu_data");
